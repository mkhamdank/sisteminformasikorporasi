...
<html lang="">
	...
	<body>
		<div class="container-fluid">
			...
			<div class="col-lg-12">
				<legend><center>Update Data Absensi</center></legend>
				<?php echo form_open('Absensi/edit/'.$this->uri->segment(3)); ?>
				<?php echo validation_errors() ?>
				<p align="right">Tanggal : <?php echo date("d-m-Y") ?></p>
					<div class="form-group">
						<?php foreach ($absensi as $key) { ?>
							<label for="">Nama Pegawai</label>
							<select name="fk_user" id="" class="form-control" required="required">
							<option value="<?php echo $key->fk_user ?>">--- <?php echo $key->nama ?> ---</option>
							<?php foreach ($sdm as $val) { ?>
								<option value="<?php echo $val->id_user ?>"><?php echo $val->nama ?></option>
							<?php } ?>
							</select>
							<label for="">Tanggal</label>
							<input type="date" name="tanggal" id="" class="form-control" value="<?php echo $key->tanggal ?>">
							<label for="">Jam Datang</label>
							<input type="time" name="jam_datang" id="inputJam_datang" class="form-control" value="<?php echo $key->jam_datang ?>">
							<label for="">Jam Pulang</label>
							<input type="time" name="jam_pulang" id="inputJam_datang" class="form-control" value="<?php echo $key->jam_pulang ?>">
							<label>Keterangan</label>
							<input type="text" name="keterangan" id="inputHidden" class="form-control" placeholder="Keterangan" value="<?php echo $key->keterangan ?>">
						<?php } ?>
					</div>
