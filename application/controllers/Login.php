	...
	public function tombollogin($password)
	{
		...
		elseif($result)
		{
			foreach ($result as $row) {
				$session_variables=array('id_user' =>$row->id_user, 'username' =>$row->username, 'password' =>$row->password, 'nama' =>$row->nama, 'alamat' =>$row->alamat, 'no_hp' =>$row->no_hp);
			}
		...
		{
			$this->form_validation->set_message('Username dan Password Salah!');
			return false;
		}
	}
